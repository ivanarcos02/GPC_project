let renderer, scene, camera, cameraControls, miniMapCamera, miniMapRenderer;
...
function init() {
  ...
  miniMapRenderer.setSize(window.innerWidth / 4, window.innerWidth / 4); // 1/4 del tamaño de la vista general
  miniMapRenderer.setClearColor(new THREE.Color(0xFFFFFF)); // Fondo blanco para el minimapa
  miniMapRenderer.domElement.style.position = "absolute";
  miniMapRenderer.domElement.style.top = "0px";
  ...
  miniMapCamera = new THREE.PerspectiveCamera(50, 1, 0.1, 2000); // Proporciones cuadradas 1:1
...
function render() {
    ...
    // Renderiza la escena desde la cámara principal
    cameraControls.update(); // Actualiza los controles de la cámara
    renderer.render(scene, camera);
  
    // Renderiza la escena desde la cámara cenital (minimapa)
    miniMapRenderer.render(scene, miniMapCamera);
  }

    // Función de actualización de la relación de aspecto
